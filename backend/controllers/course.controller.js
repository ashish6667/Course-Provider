...
export const updateCourse = async (req, res) => {
    ...
    try { 
          ...
          },
        {
            ...
            price,
            image: {
                public_id: image?.publice_id,
                url: image?.url,
            },
        }
    );
    if(!course){
        returnres.status(404).json({errors:"can't update, created by other admin"})
    }
...
export const getCourses = async (req, res) =>{
    try {
        ...
        res.status(500).json({ errors: "Error in getting coures"});
        console.log("error to get coures", error);
    }
};
...
export const buyCourses = async (req, res) => {
    ...
    try {
         ...
         // stripe payment code implementation.!!!
