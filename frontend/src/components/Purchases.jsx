...
function Purchases() {
  ...
  useEffect(() => {
    ...
    const fetchPurchases = async () => {
      try {
        ...
        // Merge purchased courses with their details
        const purchasedCourses = response.data.purchased.map((purchase) => {
          const course = response.data.courseData.find((c) => c._id === purchase.courseId);
          return course ? { ...course, purchaseId: purchase._id } : null;
  ...
  return (
    <div className="flex h-screen">
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 bg-gray-100 p-5 transition-transform duration-300 ease-in-out w-64 z-50 ${
      ...
      {/* Main Content */}
      <div className="flex-1 p-8 bg-gray-50 transition-all duration-300 md:ml-64">
